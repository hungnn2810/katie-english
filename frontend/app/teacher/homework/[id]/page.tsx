...
const TYPE_META: Record<HomeworkType, { icon: React.ElementType; color: string; bg: string }> = {
  ...
  VOCABULARY: { icon: BookOpen,   color: '#FFB26B', bg: '#FFB26B18' },
...
export default function TeacherHomeworkDetailPage() {
  const t = useTranslations('teacher.homeworkDetail');
  const tType = useTranslations('teacher.homeworkDetail.typeMeta');
  ...
  if (!hw) return (
    <Typography sx={{ color: 'text.secondary', py: 8, textAlign: 'center' }}>{t('loading')}</Typography>
  );

  const meta = TYPE_META[hw.type];
  const typeLabel = tType(hw.type);
  const now = new Date();
    ...
    <Box sx={{ maxWidth: 672 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, fontSize: 14 }}>
          ...
          <Typography sx={{ color: 'text.primary', fontWeight: 500, fontSize: 14 }}>{hw.name || typeLabel}</Typography>
        </Box>
        <Button
      ...
      <Box sx={{ bgcolor: 'white', borderRadius: 4, border: '1px solid', borderColor: 'divider', boxShadow: 1, p: 2.5, mb: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'start', gap: 2 }}>
          ...
          <Box sx={{ flex: 1, minWidth: 0 }}>
            ...
            <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>{t('createdOn', { date: new Date(hw.createdAt).toLocaleDateString() })}</Typography>
      ...
      {hw.assignments.length > 0 && (
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 1.5, mb: 3 }}>
          {([
            { icon: BarChart3,    label: t('stats.assignments'), value: hw.assignments.length, color: colors.primary },
            { icon: Clock,        label: t('stats.active'),      value: activeAssignments.length, color: '#10B981' },
            { icon: Users,        label: t('stats.enrolled'),    value: totalEnrolled, color: colors.purple },
            { icon: CheckCircle2, label: t('stats.submitted'),   value: submittedCount, color: '#22C55E' },
          ...
          {hw.assignments.map((a) => {
            ...
            return (
              <Box key={a.id} sx={{ bgcolor: 'white', borderRadius: 4, border: '1px solid', borderColor: 'divider', boxShadow: 1, overflow: 'hidden' }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, px: 2.5, py: 2, borderBottom: '1px solid', borderColor: 'divider' }}>
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, fontSize: 12, color: 'text.secondary' }}>
                      <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>{t('submittedOfShort', { submitted: submittedStudentIds.size, total: aEnrolled })}</Typography>
                      <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>·</Typography>
                      <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>{t('dueLabel', { date: formatDate(a.endDate) })}</Typography>
                    </Box>
                  </Box>
                  {deletingAssignmentId === a.id ? (
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
                      <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>{t('removeConfirm')}</Typography>
                      <Button size="small" onClick={() => setDeletingAssignmentId(null)} sx={{ fontSize: 12, borderRadius: 2, color: 'text.secondary', minWidth: 0, px: 1 }}>{t('cancel')}</Button>
                      <Button size="small" variant="contained" disabled={submitting} onClick={async () => { setSubmitting(true); try { await deleteAssignment(a.id); setDeletingAssignmentId(null); load(); } catch { setDeletingAssignmentId(null); } finally { setSubmitting(false); } }} sx={{ fontSize: 12, borderRadius: 2, bgcolor: 'error.main', '&:hover': { bgcolor: 'error.dark' }, minWidth: 0, px: 1 }}>{submitting ? '…' : t('yes')}</Button>
                    </Box>
                  ) : (
                    <Box
                      component="button"
                ...
                {/* Sessions list */}
                {sessions.length === 0 ? (
                  <Typography sx={{ px: 2.5, py: 2, fontSize: 14, color: 'rgba(100,116,139,0.6)', fontStyle: 'italic' }}>{t('noSubmissionsYet')}</Typography>
                ) : (
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                          ...
                          <Box>
                            ...
                            <Typography sx={{ color: 'text.secondary', fontSize: 12 }}>
                              {s.completedAt
                                ? t('completedAt', { date: new Date(s.completedAt).toLocaleString() })
                                : t('startedInProgress', { date: new Date(s.startedAt).toLocaleString() })}
                            </Typography>
                          </Box>
                        </Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
                ...
                {notSubmitted.length > 0 && (
                  <Box sx={{ px: 2.5, py: 1.5, borderTop: '1px solid', borderColor: 'divider', bgcolor: 'rgba(247,249,252,0.4)' }}>
                    <Typography sx={{ fontSize: 10, fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: '0.05em', mb: 1 }}>
                      {t('notSubmittedCount', { count: notSubmitted.length })}
                    </Typography>
